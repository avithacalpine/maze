...
class TBT_RewardsApi_Model_Api extends Mage_Sales_Model_Api_Resource {
    ...
    public function getTransferList($parameters = null) {
        $this->_noStFault();
        /** @var $transfer TBT_Rewards_Model_Transfer */
        $transfer = Mage::getModel('rewards/transfer');
        $transfers = $transfer->getCollection();

        if (isset($parameters['updated_at']) && isset($parameters['updated_at']['from'])) {
            $transfers->addFieldToFilter('last_update_ts', array('from' => $parameters['updated_at']['from']));
        }

        if (isset($parameters['_setPageSize'])) {
            $transfers->setPageSize($parameters['_setPageSize']);
        }
